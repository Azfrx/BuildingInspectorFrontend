				...
				<view v-for="type in ['上部结构', '下部结构', '桥面系', '附属设施']" :key="type" class="type-group">
					...
					<template v-if="getFilteredDiseasesByType(type).length > 0">
						...
						<view v-show="expandedTypes[type]">
							<disease-item
								v-for="(item, itemIndex) in getFilteredDiseasesByType(type).sort((a, b) => (a.component?.code || '') > (b.component?.code || '') ? 1 : -1)"
								:key="itemIndex" :item="item" :editMode="'history'" :selectMode="isSelectMode"
...
<script setup>
	...
	const handleNavigateHistoryDisease = (data) => {
		...
		// 获取当前选中的年份
		const selectedYear = tabItems.value[activeTab.value];
		const yearDiseases = diseaseMap.value[selectedYear] || [];

		// 查找当前病害在列表中的索引
		const currentIndex = yearDiseases.findIndex(disease => disease.id === currentId);
		if (currentIndex === -1) {
			console.warn(`未在${selectedYear}年找到ID为${currentId}的病害`);
			return;
		}
		...
		// 导航到目标病害的详情页
		uni.redirectTo({
			url: `/pages/add-disease/add-disease?mode=history&data=${encodeURIComponent(JSON.stringify(targetDisease))}`
		});
	};
</script>
