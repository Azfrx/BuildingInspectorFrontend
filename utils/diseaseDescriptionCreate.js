function generateDiseaseDescription(data) {
	...
	if (counts < 10) {
		const details = defects.map(item => {
			...
			if(showColumns[8] == 1) {descriptionArr.push(`距${item.reference1Location} ${item.reference1LocationStart}m`)}
			if(showColumns[9] == 1) {descriptionArr.push(`距${item.reference2Location} ${item.reference2LocationStart}m`)}
