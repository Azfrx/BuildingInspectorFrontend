function generateDiseaseDescription(data) {
	...
	if (counts < 10) {
		const details = defects.map(item => {
			...
			if(showColumns[4] == 1){ descriptionArr.push(`缝宽：${item.crackWidth}mm`) }
			if(showColumns[5] == 1){ descriptionArr.push(`高度/深度：${item.heightDepth}m`) }
			if(showColumns[6] == 1){ descriptionArr.push(`面积：${item.areaLength}×${item.areaWidth}m²`) }
			if(showColumns[7] == 1){ descriptionArr.push(`变形/位移：${item.deformation}m`) }
			if(showColumns[8] == 1){ descriptionArr.push(`角度：${item.angle}度`) }
			if(showColumns[9] == 1){ descriptionArr.push(`比例：${item.numeratorRatio}/${item.denominatorRatio}`) }
		...
		if(showColumns[4] == 1){ descriptionArr.push(`缝宽：${item.crackWidthRangeStart}~${item.crackWidthRangeEnd}mm`) }
		if(showColumns[5] == 1){ descriptionArr.push(`高度/深度：${item.heightDepthRangeStart}~${item.heightDepthRangeEnd}m`) }
		if(showColumns[6] == 1){ descriptionArr.push(`面积：${item.areaLength}×${item.areaWidth}m²`) }
		if(showColumns[7] == 1){ descriptionArr.push(`变形/位移：${item.deformationRangeStart}~${item.deformationRangeEnd}m`) }
		if(showColumns[8] == 1){ descriptionArr.push(`角度：${item.angleRangeStart}~${item.angleRangeEnd}度`) }
		if(showColumns[9] == 1){ descriptionArr.push(`比例：${item.numeratorRatio}/${item.denominatorRatio}`) }
